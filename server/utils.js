...
// implement send email about order format with nodeMailer

export const payOrderEmailTemplate = (order, user) => {
    const body = `<h1>Thanks for shopping with us</h1>
    <p>
    Hi ${user.name},</p>
    ...
    transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
            ...
            console.log("Email sent:" + info.response);
            res.status(201).json({ status: 201, info, message: 'We sent email summary order.' })
        }
    })
    return;
};